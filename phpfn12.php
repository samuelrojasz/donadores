...
function CurrentProjectID() {
	...
	return "{A9B917F6-72DB-4C37-BB0D-F508A0EFFBF8}";
}
	...
	function ValidateUser(&$usr, &$pwd, $autologin, $encrypted = FALSE) {
		...
		if (!$ValidateUser) {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$ValidateUser = $CustomValidateUser || ew_ComparePassword($rs->fields('passMD5'), $pwd, $encrypted);
					if ($ValidateUser) {
						$_SESSION[EW_SESSION_STATUS] = "login";
						...
						if (is_null($rs->fields('groupID'))) {
							...
							$this->setSessionUserLevelID(intval($rs->fields('groupID'))); // Load User Level
						}
						$this->SetUpUserLevel();
